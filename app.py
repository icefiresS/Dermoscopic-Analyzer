...
def post():
    ...
    class_, introduction = analyzer(target)

    return '病种: %s (%s) </p><p>' % (class_, introduction)
...
<h3>Dermatoscope Image Analyzer</h3>
<p>点击上传按钮或拖拽图片到虚线框区域. 刷新后页面上将会保留 %s 张图片.</p>
<p class="notice">上传图片后后端将会对图片进行存储和分析, 并会在5~10秒内返回结果, 在此过程中请耐心等待.</p>
<noscript>Note: You must have javascript enabled in order to upload and
dynamically view new images.</noscript>
<fieldset>
  <h3>Uploaded Dermatoscope Images (updated in real-time)</h3>
  <div id="images">%s</div>
...
<script>
  function sse() {
      ...
      source.onmessage = function(e) {
          ...
          var upload_message = 'Image uploaded by user.';
  ...
  function file_select_handler(to_upload) {
      ...
      xhr.onreadystatechange = function(e1) {
          if (this.readyState == 4)  {
              if (this.status == 200)
                  var text =  this.responseText + 'upload complete: success';
              else
